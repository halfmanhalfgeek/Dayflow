...
enum LLMOutputLanguagePreferences {
    ...
    static func languageInstruction(forJSON: Bool) -> String? {
        ...
        let verbatimClause = "If any rule requires an exact English phrase (e.g., \"Scattered apps and sites\"), keep it verbatim."
        if forJSON {
            return "The user only speaks \(lang). Respond in \(lang), but keep JSON keys in English exactly as specified. \(verbatimClause)"
        }
